...
class Tag_model extends CI_Model {
	...
	public function fetch_similar_tags( $tag_term ) {
		$this->db->like( 'tag_name', trim($tag_term ));
		$query = $this->db->get( 'tags' )->result();
		foreach ( $query as $key => $item ) {
			$query[ $key ]->created_by = $this->db->get_where( 'users', [ 'id' => $item->created_by ] )->result()[0];
			unset( $query[ $key ]->created_by->pass );
			$query[ $key ]->created_by->id = (int) $query[ $key ]->created_by->id;
			$query[ $key ]->id = (int) $query[ $key ]->id;
		}
